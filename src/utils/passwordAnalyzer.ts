...
const commonPasswords = [
  ...
  '1234', 'monkey', '1234567890', 'master', 'sunshine'
];
...
export const analyzePassword = (password: string): PasswordAnalysis => {
  ...
  const suggestions = generateSuggestions({
    ...
    hasNumber, hasSpecialChar, hasCommonPassword,
    hasSequentialChars, hasRepeatedChars
  });
...
const checkForSequentialChars = (password: string): boolean => {
  ...
  for (let i = 0; i < allSequences.length; i++) {
    ...
    for (let j = 0; j < seq.length - 2; j++) {
      ...
      if (lowerPass.includes(triplet)) return true;
    }
  }
...
// Check for 3 or more repeated characters (like "aaa" or "111")
...
const generateFeedback = (issues: any): { positives: string[], negatives: string[] } => {
...
export const getAIEnhancedSuggestions = (password: string): string[] => {
  ...
  if (password.length > 0) {
    ...
    if (/[0-9]+$/.test(password)) {
      aiSuggestions.push("Avoid placing all numbers at the end of your password");
    }
    
    if (/^[A-Z]/.test(password) && password.length > 1 && /[a-z]+$/.test(password.slice(1))) {
      aiSuggestions.push("Avoid using only an uppercase first letter followed by all lowercase");
    }
    
    // Check for dictionary word patterns
    if (/^[a-zA-Z]+$/.test(password) && password.length > 3) {
      aiSuggestions.push("Consider adding non-alphabetic characters to strengthen your password");
    }
    
    // Add personalized suggestion
    if (basicAnalysis.score < 60) {
      aiSuggestions.push("Try creating a phrase from the first letters of a memorable sentence");
    }
  }
