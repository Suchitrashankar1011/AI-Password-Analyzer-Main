...
const AnalysisResult: React.FC<AnalysisResultProps> = ({
  ...
  const hasCommonPattern = analysis.issues.hasCommonPassword;
  const sequentialChars = analysis.issues.hasSequentialChars;
  const repeatedChars = analysis.issues.hasRepeatedChars;

  // Calculate entropy based on the actual password characteristics
  const entropy = Math.round(analysis.score * 0.7); // Approximating entropy from score
  const crackTime = getCrackTimeEstimate(analysis.score);
    ...
    >
      ...
      <div className="p-5 space-y-6">
        <motion.div variants={item} className="space-y-3">
            ...
            <ul className="space-y-2 grid grid-cols-1 sm:grid-cols-2 gap-2">
              ...
              >
                ...
                <span className="text-sm">
                  Length: {analysis.issues.hasLength ? "8+" : "Less than 8"} characters {!analysis.issues.hasLength && "(recommended: 12+)"}
                </span>
              </motion.li>
              ...
              <motion.li 
                ...
                whileHover={{ scale: 1.02, transition: { duration: 0.2 } }}
              >
                {analysis.issues.hasSpecialChar ? (
                  <div className="h-6 w-6 rounded-full bg-green-500/20 flex items-center justify-center">
                    <Check className="h-4 w-4 text-green-500" />
                  </div>
                ) : (
          ...
          <motion.div variants={item} className="space-y-3">
            <h3 className="font-medium text-sm uppercase tracking-wider text-muted-foreground">Vulnerabilities:</h3>
            <ul className="space-y-2">
              ...
              {!hasCommonPattern && !sequentialChars && !repeatedChars && (
                ...
                >
                  ...
                  <span className="text-sm">No common vulnerabilities detected</span>
                </motion.li>
              )}
            </ul>
          </motion.div>
          ...
          <motion.div variants={item} className="space-y-3">
            <h3 className="font-medium text-sm uppercase tracking-wider text-muted-foreground">Security Metrics:</h3>
            <div className="grid grid-cols-2 gap-4">
              ...
              >
                ...
                <div className="text-lg font-bold text-blue-600 dark:text-blue-400">{crackTime}</div>
              </motion.div>
            </div>
          </motion.div>
      </div>
